...
#define HL_WFB_TX_RANDOM_FLIE    "/dev/random"
#define HL_WFB_TX_CLIENT_ADDR   "127.0.0.1"
...
typedef struct wfb_mng_tx {
    uint64_t epoch;// 
    uint8_t k, n, radio_port, flags;// 
    uint32_t channel_id, link_id;//
    int bandwidth, short_gi, stbc, ldpc, mcs_index, debug_port, poll_timeout;
    int s32_Rate, s32_TestTime;// 
    std::string client_addr, keypair;// 
    int udp_port, srv_port, log_interval;

    std::vector<int> rx_fd;
    std::vector<string> wlans;
...
protected:
    ...
    virtual void inject_cts_packet() = 0;
    virtual void inject_rts_packet() = 0;
...
private:
    ...
    virtual void inject_cts_packet();
    virtual void inject_rts_packet();
